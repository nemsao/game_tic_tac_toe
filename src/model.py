...
class Model(nn.Module):
    ...
    @eval
    @torch.no_grad()
    def predict(self, state: torch.Tensor, noise_level: float = 0.1) -> int:
        ...
        noise = torch.randn_like(prediction) * noise_level  # Generate noise
        noisy_prediction = prediction + noise
        action = torch.argmax(noisy_prediction, dim=-1).item()
        return action
